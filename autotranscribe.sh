...
# built off https://github.com/jessicakay/glossy/blob/main/livestream_rip.sh
# version 2
#
...
terminal -c watch -g -n 1 --no-title grep -oE 'outNAME[0-9]+.mp4' $outNAME.m3u8 && \
tail -n 1 $outNAME.m3u8 | whisper --language English --model base.en --verbose True --task transcribe --output_format txt

targURL=$(curl -L $targ | tr "\'" "\n" |
	grep "\Khttp.*?media.*?m3u?8" -Poz | tr -d '\0') &&
	ffmpeg -i $targURL -c copy -segment_time $timestamp -reset_timestamps 1 \
	-segment_list $outNAME.m3u8	\ # creates auto-updating playlist for watch -g
